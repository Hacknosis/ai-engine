...
def draw_mask(image, mask, color, alpha=0.5):
    ...
    overlay = image.copy()
    image[mask] = (1 - alpha) * image[mask] + alpha * np.array(color) * 255
    return image
...
def MedSAM_infer_npz_2D(img_npz_file):
    ...
    for i, box in enumerate(boxes):
        ...
        box_viz = resize_box_to_256(box, original_size=(H, W))
        draw_box(overlay, box_viz, [0, 215, 255], thickness=2)
        mask = (segs == i + 1).astype(np.uint8)
        overlay = draw_mask(overlay, mask, color, alpha=0.5)
    print("saving")
    cv2.imwrite(join(pred_save_dir, npz_name.split(".")[0] + '.png'), overlay)
